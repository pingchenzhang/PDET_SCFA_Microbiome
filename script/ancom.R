...
ANCOM = function(feature_table, meta_data, struc_zero = NULL, main_var, p_adj_method = "BH", 
  ...
  # data.frame(y = clr_table[1,], 
  #            x = meta_data %>% pull(main_var))
  # lm(y ~ x, data = data.frame(y = clr_table[1,], 
  #                             x = meta_data %>% pull(main_var),
  #                             check.names = FALSE))$coef
  # mean(clr_table[1,1:2]) - mean(clr_table[1,3:4])
  # mean(clr_table[1,5:6]) - mean(clr_table[1,3:4])
  # mean(clr_table[1,7:8]) - mean(clr_table[1,3:4])
  ...
  plotvoc <- function(){
    ...
    data.frame(combn(group_names, 2), stringsAsFactors = F) %>%
      purrr::map_dfr(~{
        list(
          taxa_id = rownames(clr_table),
          clr_mdiff= lmeans[[.x[2]]]-lmeans[[.x[1]]],
          group  = paste(.x[2], ' vs ', .x[1]))
      }) %>%
      dplyr::left_join(dplyr::select(out,taxa_id,W), by=c("taxa_id")) %>%
      dplyr::mutate(zero_ind = factor(ifelse(is.infinite(W), "Yes", "No"), levels = c("Yes", "No"))) %>%
      dplyr::mutate(W = replace(W, is.infinite(W), n_taxa - 1)) -> fig_dat
    # Replace Inf by (n_taxa - 1) for structural zeros

    ggplot(data = fig_dat) + 
      geom_point(aes(x = clr_mdiff , y = W)) + 
      facet_wrap(~ group) +
      labs(x = "CLR mean difference", y = "W statistic") +
      # scale_color_discrete(name = "Structural zero", drop = FALSE) + 
      theme_bw() +
  ...
  names(cut_off) = c("0.9", "0.8", "0.7", "0.6")
  p <- plotvoc()
  addw <- function(p, cutW) {
    p + geom_hline(yintercept = cutW, linetype = "dashed")+
      geom_point(aes(x = clr_mdiff, y = W, color=taxa_id),
                 data = dplyr::filter(p$data, W>cutW)) + 
      ggplot2::annotate('text',x = min(p$data$clr_mdiff), y=cutW,
                        label=paste0('W[',names(cutW),']'), size = 4, vjust = -0.5, hjust = 0,
                        color = "orange", parse = TRUE) +
      guides(color= guide_legend(title = legend_name))
  }
  p7=addw(p, cut_off['0.7'])
  ## res
  res = list(p_data = p_data, q_data = q_data, out = out,
             fig = fig, cut_off=cut_off, addw=addw,
             p=p, p7=p7)
  return(res)
}
...
run_ancom <- function(df, sgc, opre=NULL, legend_name=''){
  ...
  feature_table = df[, names(sgc$sg)]; group_var = NULL;
  ...
  list(
    data=list(
      Figdata= res$p$data,
      Result = res$out,
      feature_table = tibble::rownames_to_column(feature_table,'taxa_id'),
      meta_data = meta_data
    ),
    res=res,
    plotn = length(unique(res$p$data$group))
  ) -> ot
  if(!is.null(opre)){
    dir.create(dirname(opre),showWarnings = F, recursive = T)
    saveRDS(ot, paste0(opre,'.rds'))
    writexl::write_xlsx(ot$data, paste0(opre, '.xlsx'))
    ggsave(paste0(opre,'.pdf'), res$p7,device = cairo_pdf,
           width = 5.5+sqrt(ot$plotn), height = 4+sqrt(ot$plotn))
    cli::cli_alert_success('{opre} .pdf .xlsx')
  }else{ return(ot) }
}
